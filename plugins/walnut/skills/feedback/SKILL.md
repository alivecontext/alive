---
description: "Report a bug, request a feature, or send general feedback to the walnut team. Collects safe system metadata, shows a preview, and creates a GitHub Issue. Nothing personal leaves the machine — only what you type and anonymous system info."
user-invocable: true
---

# Feedback

Send feedback to the walnut team. Bug reports, feature requests, general thoughts — whatever is on your mind.
...
Check if a `/walnut:*` skill was invoked earlier in this session. If so, offer to include it:

```
╭─ 🐿️ context
│  Looks like you were using walnut:save
│
...
| Engine/model | You know which model you are — state it (e.g., `claude-opus-4-6`) |
| OS/platform | Run: `uname -s` via Bash |
| Context usage % | Read `.walnut/.context_pct` if it exists (written by statusline). Fallback: `.alive/.context_pct` (pre-migration worlds). Otherwise: "unavailable" |
| Session duration | Estimate from `started:` field in the squirrel YAML at `.walnut/_squirrels/{session_id}.yaml`. Fallback: `.alive/_squirrels/`. Calculate minutes elapsed from then to now. If unavailable: "unavailable" |

**Never collect:** walnut contents, file paths, conversation history, PII, credentials, `.walnut/key.md`.
...
│  Labels: feedback, bug
│
│  → this will be posted publicly to github.com/stackwalnuts/walnut
│
│  ▸ Send?
│  1. Send
│  2. Edit description
...
```bash
gh issue create \
  --repo stackwalnuts/walnut \
  --title "[Bug] Save skill dropped stash items" \
  --label "feedback,bug" \
  --body "$(cat <<'WALNUT_FEEDBACK_EOF'
### Description
...
```
╭─ 🐿️ sent
│  Feedback submitted: https://github.com/stackwalnuts/walnut/issues/42
│  Thanks — back to work.
╰─
```
...
│
│  Two options:
│  1. Run `gh auth login` in your terminal, then try /walnut:feedback again
│  2. Copy the issue below and paste it at:
│     github.com/stackwalnuts/walnut/issues/new
│
│  ---
...
- Read any walnut file contents (key.md, now.md, log.md, insights.md, tasks.md)
...
- Include `.walnut/key.md` or world identity
...
The squirrel can suggest `/walnut:feedback` when it notices problems — but only if `feedback_nudges` is enabled in `.walnut/preferences.yaml` (default: true).
...
```
╭─ 🐿️ that hook failed
│  Want to report it? /walnut:feedback
╰─
```
