...
Not a search (that's find — searches content across walnuts). History searches SESSIONS — what squirrels did, what was discussed, what was decided. For deep context extraction from source material, use `walnut:mine`.
...
**Tier 1 — Squirrel Entries** (`.walnut/_squirrels/*.yaml`, with fallback to per-walnut `_core/_squirrels/*.yaml`)
...
After listing sessions, check for unsaved entries (`saves: 0`) in `.walnut/_squirrels/`. These represent sessions that weren't properly closed — the human may have lost stash items.
...
```
╭─ 🐿️ history — combining 3 sessions
│
│  Loading:
...
That last question is key: **the handoff is targeted.** Not just dumping everything. The system builds the context pack around WHY it's needed. "I need to write the investor deck" gets different context emphasis than "I need to debug the hook scripts."
...
- Current state (from `_core/now.md`)
- Open questions (from stash + conversation)
- Capsule drafts in progress
- Key relationships and people context
- The specific reason for the handoff

Written to `_core/_capsules/_inbox/raw/recall-[date]-[topic].md` and optionally loaded directly into a new session.

```
╭─ 🐿️ handoff ready
│
│  _core/_capsules/_inbox/raw/recall-2026-02-24-investor-deck.md
│
│  Context from 3 sessions, focused on: investor deck preparation
│  12 decisions, 4 open questions, 3 capsules referenced
│
...
When history finds sessions with rich, unmined context — long sessions, many stash items, transcripts with extensive research or decision-making — suggest escalation to `walnut:mine`:

```
╭─ 🐿️ these sessions look rich
│  3 sessions from last week have 45+ stash items and long transcripts.
│  Want to deep-mine them for people, decisions, and patterns?
│
│  → Mine them (walnut:mine)
│  → Just the summary is fine
╰─
```

This is the bridge between "what happened?" (history) and "what's buried in there?" (mine).

---

## Transcript Discovery

Different platforms store session data in different places. Resolve the transcript path before dispatching revive agents.
...
**Privacy note:** Transcripts live on the human's machine in Claude Code's project directory. They never leave. History reads them locally.

---

## What History Is NOT

- Not `walnut:find` — find searches content across walnuts. History searches sessions.
- Not `walnut:tidy` — tidy surfaces broken things. History surfaces past context.
- Not `walnut:mine` — mine does deep extraction from source material. History is the lighter "what happened recently?"
- Not a log viewer — `_core/log.md` has the signed record. History has the full session timeline and can revive transcripts.

The log tells you WHAT was decided. History tells you WHEN, and can revive the WHY from transcripts.
