...
templates/bundle/context.manifest.yaml → bundles/{name}/context.manifest.yaml
```
...
- **Phase** — starting, planning, building, testing, launched → now.json `phase:`
...
Follow the process from `world.md` exactly:

1. Domain already determined (Step 1-2 above)
2. Create folder at the resolved path (kebab-case name)
3. Read each template from `templates/walnut/`, fill `{{placeholders}}`, write to `_kernel/` inside the walnut. If `has_codebase: true`, use `templates/walnut/key-codebase.md` instead of `templates/walnut/key.md` — fill the `dev:` block with repo, local_path, and any optional fields collected in Step 2b
4. Create empty directory: `bundles/`
...
8. If sub-walnut: set `parent: [[parent-name]]` in `_kernel/key.md` frontmatter
9. Add `[[new-walnut-name]]` to parent's `_kernel/key.md` `links:` frontmatter field
10. Update `.alive/key.md` — add the new walnut to `## Connections`: `- [[new-walnut-name]] — {goal}`. If `.alive/_index.yaml` exists, regenerate it too.
...
|------|----------------|
| `{domain}/{name}/_kernel/key.md` | `templates/walnut/key.md` |
| `{domain}/{name}/_kernel/_generated/now.json` | `templates/walnut/now.json` |
| `{domain}/{name}/_kernel/log.md` | `templates/walnut/log.md` |
| `{domain}/{name}/_kernel/insights.md` | `templates/walnut/insights.md` |
| `{domain}/{name}/bundles/` | Empty directory |
...
| `templates/walnut/now.json` | Template — read before writing now.json |
...
| `templates/bundle/context.manifest.yaml` | Template — read before writing bundle manifest |
